UPDATE 1.07.01: 2023.11.05 ddr
	1) Добавлена форма для ввода данных в колонку
	2) Добавлен ивент dataGridView1_CellDoubleClick для вызова формы для ввода данных.
UPDATE 1.07.00: 2023.10.24 ddr
	1) закончил функционал финальной таблицы с оценками.
	2) выставил инвариантную культуру (ДОЛЖНО РАБОТАТЬ :))
	3) Пофиксил баг у добавлением множества работ через окно, теперь не вешается и не ловит оверфлоу.

UPDATE 1.06.04: 2023.10.22 agp
       1)
            похерил это свойство с ним не строилось
       		public string DbName ; //;{ get; set; }

       2)
            заголовок сдвинут, так как имя длинное
id	Name	Add	Lab1.1	Lab1.2	lab1.3	lab1.4	Mod1
1	Apanasov Danil	0	3	0	0	0	0

       3)
-ae.jpg    --  ввод ключа без параметра
-p.jpg     --  кривая таблица
point-comma1.jpg  -- добавлял оценку 3,1 с моей культурой  -- запятая
addBall.jpg  --      добавлял целую оценку
point-comma2.jpg  --  добавлял оценку не смоей культрой -- точка
351.JPG           -- добавлял 351 группу
...
UPDATE 1.06.02: 2023.10.3 ddr
	...
	Для лучшей совместимости начиная с 1.06.02 данная ветка будет разрабатываться
 на .Net Framework (а не на .NET Core, как раньше)
	P.S. Версия с последними нововведениями языка C# будет так же получать 
все функциональные обновления и усовершенствоваться параллельно с этой.
